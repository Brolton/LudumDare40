...
public class TitleBackButton : MonoBehaviour {
	...
	void Update () {
        ...
        if (options == true)
        {
			...
			// Wwise
//			AkSoundEngine.SetRTPCValue ("RTPC_Music_Volume", music.volume); 			// Wwise set Music volume
//			AkSoundEngine.SetRTPCValue("RTPC_SFX_Volume", sfx.volume);					// Wwise set SFX volume
